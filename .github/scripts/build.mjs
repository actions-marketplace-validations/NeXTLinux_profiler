...
import url from "url"
import metadata from "../../source/app/profiler/metadata.mjs"
...
const __action = paths.join(__profiler, "source/app/action")
const __web = paths.join(__profiler, "source/app/web")
const __readme = paths.join(__profiler, ".github/readme")
const __documentation = paths.join(__profiler, ".github/readme/partials/templated")
const __templates = paths.join(paths.join(__profiler, "source/templates/"))
const __plugins = paths.join(paths.join(__profiler, "source/plugins/"))
const __test_cases = paths.join(paths.join(__profiler, "tests/cases"))
const __test_secrets = paths.join(paths.join(__profiler, "tests/secrets.json"))
...
//Example workflows
await update({source: paths.join(__profiler, ".github/scripts/files/examples.yml"), output: ".github/workflows/examples.yml", context: {steps: yaml.dump(workflow, {quotingType: '"', noCompatMode: true})}})
...
async function update({source, output, context = {}, options = {}}) {
  ...
  const file = paths.join(__profiler, output)
