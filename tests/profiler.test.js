...
action.run = async vars =>
  await new Promise((solve, reject) => {
    let [stdout, stderr] = ["", ""]
    const env = {...process.env, ...action.input(vars), GITHUB_REPOSITORY: "nextlinux/profiler"}
...
//Web instance
const web = {}
web.run = async vars => (await axios.get(`http://localhost:3000/nextlinux?${new url.URLSearchParams(Object.fromEntries(Object.entries(vars).map(([key, value]) => [key.replace(/^plugin_/, "").replace(/_/g, "."), value])))}`)).status === 200
web.start = async () =>
  new Promise(solve => {
    let stdout = ""
...
placeholder.run = async vars => {
  ...
  return typeof await placeholder({
    ...
    version: "TEST",
    user: "nextlinux",
    avatar: "https://github.com/nextlinux.png",
  }) === "string"
}
...
const metadata = JSON.parse(`${
  processes.spawnSync("node", [
    ...
    "module",
    "--eval",
    'import metadata from "./source/app/profiler/metadata.mjs";console.log(JSON.stringify(await metadata({log:false})))',
  ]).stdout
}`)
