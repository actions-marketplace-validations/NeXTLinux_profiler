...
export default async function({graphql, rest}) {
  ...
  if (mocked)
    return {graphql, rest}
  mocked = true
  process.env.profiler_MOCKED = true
  console.debug("profiler/compute/mocks > mocking")
  ...
  {
    //Unmocked
    console.debug("profiler/compute/mocks > mocking graphql api")
  ...
  {
    //Unmocked
    console.debug("profiler/compute/mocks > mocking rest api")
    const unmocked = {}
    //Mocked
    const mocker = ({path = "rest", mocks, mocked}) => {
      for (const [key, value] of Object.entries(mocks)) {
        console.debug(`profiler/compute/mocks > mocking rest api > mocking ${path}.${key}`)
  ...
  {
    //Unmocked
    console.debug("profiler/compute/mocks > mocking axios")
  ...
  {
    //Unmocked
    console.debug("profiler/compute/mocks > mocking rss-parser")

    //Mock rss feed
    rss.prototype.parseURL = function(url) {
      console.debug(`profiler/compute/mocks > mocking rss feed result > ${url}`)
      return ({
  ...
  {
    //Unmocked
    console.debug("profiler/compute/mocks > mocking google-maps-services-js")

    //Mock geocode API
    Gmap.prototype.geocode = function() {
      console.debug("profiler/compute/mocks > mocking google maps geocode result")
