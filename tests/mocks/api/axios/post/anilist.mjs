...
export default function({faker, url, body, login = faker.internet.userName()}) {
  if (/^https:..graphql.anilist.co.*$/.test(url)) {
    ...
    if (/^query Statistics /.test(query)) {
      console.debug("profiler/compute/mocks > mocking anilist api result > Statistics")
      return ({
        status: 200,
        data: {
          data: {
    ...
    if (/^query FavoritesCharacters /.test(query)) {
      console.debug("profiler/compute/mocks > mocking anilist api result > Favorites characters")
      return ({
        status: 200,
        data: {
          data: {
    ...
    if (/^query Favorites /.test(query)) {
      console.debug("profiler/compute/mocks > mocking anilist api result > Favorites")
    ...
    if (/^query Medias /.test(query)) {
      console.debug("profiler/compute/mocks > mocking anilist api result > Medias")
