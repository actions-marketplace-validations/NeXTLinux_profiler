...
export default function({faker, url, body, login = faker.internet.userName()}) {
  if (/^https:..leetcode.com.graphql.*$/.test(url)) {
    ...
    if (/^query Languages /.test(query)) {
      console.debug("profiler/compute/mocks > mocking leetcode api result > Languages")
      return ({
        status: 200,
        data: {
          data: {
    ...
    if (/^query Skills /.test(query)) {
      console.debug("profiler/compute/mocks > mocking leetcode api result > Skills")
      return ({
        status: 200,
        data: {
          data: {
    ...
    if (/^query Problems /.test(query)) {
      console.debug("profiler/compute/mocks > mocking leetcode api result > Problems")
      return ({
        status: 200,
        data: {
          data: {
    ...
    if (/^query Recent /.test(query)) {
      console.debug("profiler/compute/mocks > mocking leetcode api result > Recent")
      return ({
        status: 200,
        data: {
          data: {
