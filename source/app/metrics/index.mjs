...
//Setup
export default async function profiler({login, q}, {graphql, rest, plugins, conf, die = false, verify = false, convert = null, callbacks = null, warnings = []}, {Plugins, Templates}) {
  //Compute rendering
  try {
    //Debug
    login = login.replace(/[\n\r]/g, "")
    console.debug(`profiler/compute/${login} > start`)
    ...
    console.debug(`profiler/compute/${login} > output format set to ${convert}`)
    ...
    if (conf.settings["debug.headless"]) {
      ...
      console.debug(`profiler/compute/${login} > disabled puppeteer headless mode`)
    }
    if ((conf.settings.debug) || (process.env.GITHUB_ACTIONS)) {
      ...
      if (dflags.includes("--puppeteer-debug")) {
        ...
        console.debug(`profiler/compute/${login} > enabled puppeteer debugging`)
      }
      if (dflags.find(flag => flag.startsWith("--puppeteer-wait-"))) {
        ...
        console.debug(`profiler/compute/${login} > overridden puppeteer wait events [${imports.puppeteer.events}]`)
      }
    }

    //profiler insights
    if (convert === "insights")
      return profiler.insights.output({login, imports, conf}, {graphql, rest, Plugins, Templates})
    ...
    //Partial parts
    {
      ...
      console.debug(`profiler/compute/${login} > content order : ${[...data.partials]}`)
    }

    //Executing base plugin and compute profiler
    console.debug(`profiler/compute/${login} > compute`)
    ...
    if (errors.length) {
      console.debug(`profiler/compute/${login} > ${errors.length} errors !`)
      if (die)
    ...
    //JSON output
    if (convert === "json") {
      console.debug(`profiler/compute/${login} > json output`)
    ...
    if (/markdown/.test(convert)) {
      ...
      console.debug(`profiler/compute/${login} > markdown render`)
      let source = image
      try {
        ...
        if (!/^https:/.test(template)) {
          ...
          console.debug(`profiler/compute/${login} > on ${repo} with default branch ${branch}`)
          template = `https://raw.githubusercontent.com/${repo}/${branch}/${template}`
        }
        console.debug(`profiler/compute/${login} > fetching ${template}`)
      ...
      const embed = async (name, q = {}) => {
        //Check arguments
        console.debug(`profiler/compute/${login}/embed > ${name} >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>`)
        ...
        console.debug(`profiler/compute/${login} > embed called with`)
        console.debug(q)
        let {base} = q
        ...
        //Compute rendering
        const {rendered} = await profiler({login, q}, {...arguments[1], convert: ["svg", "png", "jpeg"].includes(q["config.output"]) ? q["config.output"] : null, warnings}, arguments[2])
        console.debug(`profiler/compute/${login}/embed > ${name} > success >>>>>>>>>>>>>>>>>>>>>>`)
        return `<img class="profiler-cacheable" data-name="${name}" src="data:image/${{png: "png", jpeg: "jpeg"}[q["config.output"]] ?? "svg+xml"};base64,${Buffer.from(rendered).toString("base64")}">`
      }
      ...
      console.debug(`profiler/compute/${login} > success`)
      //Output
      if (convert === "markdown-pdf") {
    ...
    //Rendering
    console.debug(`profiler/compute/${login} > render`)
    ...
    if ((verify) && (imports.metadata.plugins.core.extras("verify", {...conf.settings, error: false}))) {
      console.debug(`profiler/compute/${login} > verify SVG`)
      let libxmljs = null
      try {
        libxmljs = (await import("libxmljs2")).default
      }
      catch (error) {
        console.debug(`profiler/compute/${login} > failed to import libxmljs2 (${error}), ignoring SVG verification`)
      }
      if (!libxmljs) {
        const parsed = libxmljs.parseXml(rendered)
        if (parsed.errors.length)
          throw new Error(`Malformed SVG : \n${parsed.errors.join("\n")}`)
        console.debug(`profiler/compute/${login} > verified SVG, no parsing errors found`)
      }
    }
    //Resizing
...
//profiler insights
profiler.insights = async function({login}, {graphql, rest, conf, callbacks}, {Plugins, Templates}) {
  return profiler({login, q: profiler.insights.q}, {graphql, rest, plugins: profiler.insights.plugins, conf, callbacks, convert: "json"}, {Plugins, Templates})
}
profiler.insights.q = {
  template: "classic",
  achievements: true,
...
//profiler insights static render
profiler.insights.output = async function({login, imports, conf}, {graphql, rest, Plugins, Templates}) {
  //Server
  console.debug(`profiler/compute/${login} > insights`)
  const server = `http://localhost:${conf.settings.port}`
  console.debug(`profiler/compute/${login} > insights > server on port ${conf.settings.port}`)

  //Data processing
  const browser = await imports.puppeteer.launch()
  const page = await browser.newPage()
  console.debug(`profiler/compute/${login} > insights > generating data`)
  const result = await profiler.insights({login}, {graphql, rest, conf}, {Plugins, Templates})
  const json = JSON.stringify(result)
  await page.goto(`${server}/insights/${login}?embed=1&localstorage=1`)
  await page.evaluate(async json => localStorage.setItem("local.profiler", json), json) //eslint-disable-line no-undef
  await page.goto(`${server}/insights/${login}?embed=1&localstorage=1`)
  await page.waitForSelector(".container .user", {timeout: 10 * 60 * 1000})

  //Rendering
  console.debug(`profiler/compute/${login} > insights > rendering data`)
  const rendered = `
    <html>
      <head>
        <meta charset="utf-8">
        <title>profiler insights: ${login}</title>
