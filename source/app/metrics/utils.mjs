...
export async function run(command, options, {prefixed = true, log = true, debug = true} = {}) {
  ...
  return new Promise((solve, reject) => {
    if (debug)
      console.debug(`profiler/command/run > ${command}`)
    ...
    child.on("close", code => {
      if (debug)
        console.debug(`profiler/command/run > ${command} > exited with code ${code}`)
      if (log) {
...
export async function spawn(command, args = [], options = {}, {prefixed = true, timeout = 300 * 1000, stdout, debug = true} = {}) { //eslint-disable-line max-params
  ...
  return new Promise((solve, reject) => {
    if (debug)
      console.debug(`profiler/command/spawn > ${command} with ${args.join(" ")}`)
    ...
    child.on("close", async code => {
      if (debug)
        console.debug(`profiler/command/spawn > ${command} with ${args.join(" ")} > exited with code ${code}`)
      await closed
      if (debug)
        console.debug(`profiler/command/spawn > ${command} with ${args.join(" ")} > reader closed`)
...
export async function which(command) {
  try {
    console.debug(`profiler/command > checking existence of ${command}`)
    await run(`which ${command}`)
    return true
  }
  catch {
    console.debug(`profiler/command > checking existence of ${command} > failed`)
  }
  return false
}
...
export const filters = {
  /**GitHub query filter */
  github(text, object) {
    console.debug(`profiler/svg/ghquery > checking ${text} against ${JSON.stringify(object)}`)
    const result = text.split(/(?<!NOT) /).map(x => x.trim()).filter(x => x).map(criteria => {
      ...
      console.debug(`profiler/svg/ghquery > checking ${criteria} against ${value}`)
      if (value === undefined) {
        console.debug(`profiler/svg/ghquery > value for ${criteria} is undefined, considering it truthy`)
        return true
      }
    ...
    console.debug(`profiler/svg/ghquery > ${result ? "matching" : "not matching"}`)
    return result
  },
  /**Repository filter*/
  repo(repository, patterns, {debug = true} = {}) {
    ...
    if (patterns[0] === "@use.patterns") {
      if (debug)
        console.debug(`profiler/filters/repo > ${repo} > using advanced pattern matching`)
      ...
      for (let pattern of patterns) {
        ...
        if (minimatch(handle, pattern, options)) {
          if (debug)
            console.debug(`profiler/filters/repo > ${repo} matches ${action ? "including" : "excluding"} pattern ${pattern}`)
          include = action
        }
      }
    }
    //Basic pattern matching
      ...
      if (debug)
        console.debug(`profiler/filters/repo > ${repo} > using basic pattern matching`)
      include = (!patterns.includes(repo)) && (!patterns.includes(handle))
    }

    if (debug)
      console.debug(`profiler/filters/repo > filter ${repo} (${include ? "included" : "excluded"})`)
    return include
  },
  /**Text filter*/
  text(text, patterns, {debug = true} = {}) {
    ...
    if (debug)
      console.debug(`profiler/filters/text > filter ${text} (${include ? "included" : "excluded"})`)
    return include
  },
}
...
export const svg = {
  ...
  async pdf(rendered, {paddings = "", style = "", twemojis = false, gemojis = false, octicons = false, rest = null, errors = []} = {}) {
    ...
    if (!svg.resize.browser) {
      ...
      console.debug(`profiler/svg/pdf > started ${await svg.resize.browser.version()}`)
    }
    ...
    console.debug("profiler/svg/pdf > loading svg")
    const page = await svg.resize.browser.newPage()
    page.on("console", ({_text: text}) => console.debug(`profiler/svg/pdf > puppeteer > ${text}`))
    await page.setContent(`<main class="markdown-body">${rendered}</main>`, {waitUntil: puppeteer.events})
    console.debug("profiler/svg/pdf > loaded svg successfully")
    const margins = (Array.isArray(paddings) ? paddings : paddings.split(",")).join(" ")
    console.debug(`profiler/svg/pdf > margins set to ${margins}`)
    ...
    //Result
    await page.close()
    console.debug("profiler/svg/pdf > rendering complete")
  ...
  async resize(rendered, {paddings, convert, scripts = []}) {
    ...
    if (!svg.resize.browser) {
      svg.resize.browser = await puppeteer.launch()
      console.debug(`profiler/svg/resize > started ${await svg.resize.browser.version()}`)
    }
    //Format padding
    ...
    console.debug(`profiler/svg/resize > padding width*${padding.width}+${padding.absolute.width}, height*${padding.height}+${padding.absolute.height}`)
    //Render through browser and resize height
    console.debug("profiler/svg/resize > loading svg")
    const page = await svg.resize.browser.newPage()
    page.setViewport({width: 980, height: 980})
    page
      .on("console", message => console.debug(`profiler/svg/resize > puppeteer > ${message.text()}`))
      .on("pageerror", error => console.debug(`profiler/svg/resize > puppeteer > ${error.message}`))
    await page.setContent(rendered, {waitUntil: puppeteer.events})
    console.debug("profiler/svg/resize > loaded svg successfully")
    await page.addStyleTag({content: "body { margin: 0; padding: 0; }"})
    let mime = "image/svg+xml"
    console.debug("profiler/svg/resize > resizing svg")
    ...
    try {
      ;({resized, width, height} = await page.evaluate(
        async (padding, scripts) => {
          ...
          for (const script of scripts) {
            try {
              console.debug(`profiler/svg/resize > executing ${script}`)
              await new Function("document", `return (async () => {${script}})()`)(document) //eslint-disable-line no-new-func
              console.debug("profiler/svg/resize > successfully executed user javascript")
            }
            catch (error) {
          ...
          let {y: height, width} = document.querySelector("svg #profiler-end").getBoundingClientRect()
      ...
      console.debug(`profiler/svg/resize > an error occurred: ${error}`)
      throw error
    }
    //Convert if required
    if (convert) {
      console.debug(`profiler/svg/resize > convert to ${convert}`)
      resized = await page.screenshot({type: convert, clip: {x: 0, y: 0, width, height}, omitBackground: true})
      mime = `image/${convert}`
    }
    //Result
    await page.close()
    console.debug("profiler/svg/resize > rendering complete")
    return {resized, mime}
  },
  ...
  async hash(rendered) {
    ...
    if (!svg.resize.browser) {
      svg.resize.browser = await puppeteer.launch()
      console.debug(`profiler/svg/hash > started ${await svg.resize.browser.version()}`)
    }
    //Compute hash
    const page = await svg.resize.browser.newPage()
    await page.setContent(rendered, {waitUntil: puppeteer.events})
    ...
    //Result
    await page.close()
    console.debug(`profiler/svg/hash > MD5=${hash}`)
    return hash
  },
  /**Render twemojis */
  async twemojis(rendered, {custom = true} = {}) {
    //Load emojis
    console.debug("profiler/svg/twemojis > rendering twemojis")
    ...
    for (const [emoji, twemoji] of emojis) {
      if (custom)
        rendered = rendered.replace(new RegExp(`<profiler[ ]*(?<attributes>[^>]*)>${emoji}</profiler>`, "g"), twemoji.replace(/(<svg class="twemoji" [\s\S]+?)(>)/, "$1 $<attributes> $2"))
  ...
  async gemojis(rendered, {rest}) {
    //Load gemojis
    console.debug("profiler/svg/gemojis > rendering gemojis")
    const emojis = new Map()
    try {
      ...
      console.debug("profiler/svg/gemojis > could not load gemojis")
      console.debug(error)
    }
    //Apply replacements
  ...
  async octicons(rendered) {
    //Load octicons
    console.debug("profiler/svg/octicons > rendering octicons")
    const icons = new Map()
  ...
  optimize: {
    ...
    async css(rendered) {
      //Extract styles
      console.debug("profiler/svg/optimize/css > optimizing")
    ...
    async xml(rendered, {raw = false} = {}) {
      console.debug("profiler/svg/optimize/xml > optimizing")
      if (raw) {
        console.debug("profiler/svg/optimize/xml > skipped as raw option is enabled")
        return rendered
      }
    ...
    async svg(rendered, {raw = false} = {}, experimental = new Set()) {
      console.debug("profiler/svg/optimize/svg > optimizing")
      if (raw) {
        console.debug("profiler/svg/optimize/svg > skipped as raw option is enabled")
        return rendered
      }
      if (!experimental.has("--optimize")) {
        console.debug("profiler/svg/optimize/svg > this feature require experimental feature flag --optimize-svg")
        return rendered
      }
...
export async function record({page, width, height, frames, scale = 1, quality = 80, x = 0, y = 0, delay = 150, background = true}) {
  ...
  for (let i = 0; i < frames; i++) {
    ...
    if (i % 10 === 0)
      console.debug(`profiler/record > processed ${i}/${frames} frames`)
  }
  console.debug(`profiler/record > processed ${frames}/${frames} frames`)
  //Post-processing
  console.debug("profiler/record > applying post-processing")
...
export async function gif({page, width, height, frames, x = 0, y = 0, repeat = true, delay = 150, quality = 10}) {
  ...
  console.debug(`profiler/puppeteergif > set write stream to "${path}"`)
  ...
  for (let i = 0; i < frames; i++) {
    ...
    if (frames % 10 === 0)
      console.debug(`profiler/puppeteergif > processed ${i}/${frames} frames`)
  }
  console.debug(`profiler/puppeteergif > processed ${frames}/${frames} frames`)
