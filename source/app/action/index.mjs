...
;(async function() {
  try {
    ...
    if ((github.context.eventName === "push") && (github.context.payload?.head_commit)) {
      ...
      if (/Auto-generated profiler for run #\d+/.test(github.context.payload.head_commit.message)) {
    ...
    if (!token)
      throw new Error("You must provide a valid GitHub personal token to gather your profiler (see https://github.com/lowlighter/profiler/blob/master/.github/readme/partials/documentation/setup/action.md for more informations)")
    ...
    //Test token validity and requests count
      ...
      if (ratelimit) {
        ...
        info.break()
        console.log("Nothing can be done currently, thanks for using profiler!")
        quit("skipped")
      }
      //Check scopes
      try {
        ...
        if (!("x-oauth-scopes" in headers)) {
          throw new Error(
            'GitHub API did not send any "x-oauth-scopes" header back from provided "token". It means that your token may not be valid or you\'re using GITHUB_TOKEN which cannot be used since profiler will fetch data outside of this repository scope. Use a personal access token instead (see https://github.com/lowlighter/profiler/blob/master/.github/readme/partials/documentation/setup/action.md for more informations).',
          )
        }
    ...
    if (_notice_releases) {
      const {data: [{tag_name: tag}]} = await rest.repos.listReleases({owner: "lowlighter", repo: "profiler"})
      const current = Number(conf.package.version.match(/(\d+\.\d+)/)?.[1] ?? 0)
      const latest = Number(tag.match(/(\d+\.\d+)/)?.[1] ?? 0)
      if (latest > current)
        console.info(`::notice::A new version of profiler (v${latest}) has been released, check it out for even more features!`)
    }
    ...
    if ((!dryrun) && (_action !== "none")) {
      ...
      committer.head = committer.pr ? `profiler-run-${github.context.runId}` : committer.branch
      info("Committer token", committer.token, {token: true})
      if (!committer.token)
        throw new Error("You must provide a valid GitHub token to commit your profiler")
    ...
    if (/insights/.test(convert)) {
      try {
        await new Promise(async (solve, reject) => {
          ...
          const web = await processes.spawn("node", ["/profiler/source/app/web/index.mjs"], {env: {...process.env}})
    ...
    let {rendered, mime} = await retry(async () => {
      const {rendered, mime, errors} = await profiler({login: user, q}, {graphql, rest, plugins, conf, die, verify, convert}, {Plugins, Templates})
      if (errors.length) {
    ...
    if (!rendered)
      throw new Error("Could not render profiler")
    ...
    if (_output === "svg") {
      ...
      if ((_output_condition === "data-changed") && ((committer.commit) || (committer.pr))) {
        const {svg} = await import("../profiler/utils.mjs")
        let data = ""
        await retry(async () => {
          try {
    ...
    if (dryrun)
      ...
      info(`Save to /profiler_renders/${filename}`, "ok")
    ...
    if (_action === "none") {
      info.break()
      console.log("Success, thanks for using profiler!")
    }
    //Perform output action
    else {
      //Cache embed svg for markdown outputs
      if (/markdown/.test(convert)) {
        const regex = /(?<match><img class="profiler-cacheable" data-name="(?<name>[\s\S]+?)" src="data:image[/](?<format>(?:svg[+]xml)|jpeg|png);base64,(?<content>[/+=\w]+?)">)/
        let matched = null
        ...
        buffer.content = rendered
        await fs.writeFile(paths.join("/renders", filename), buffer.content)
        info(`Update /profiler_renders/${filename}`, "ok")
      }
      ...
      if (committer.pr) {
        ...
        await retry(async () => {
          try {
            ;({data: {number}} = await committer.rest.pulls.create({...github.context.repo, head: committer.head, base: committer.branch, title: `Auto-generated profiler for run #${github.context.runId}`, body: " ", maintainer_can_modify: true}))
            ...
            if (/A pull request already exists/.test(error)) {
              ...
              const q = `repo:${github.context.repo.owner}/${github.context.repo.repo}+type:pr+state:open+Auto-generated profiler for run #${github.context.runId}+in:title`
