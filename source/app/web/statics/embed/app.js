;(async function() {
  ...
  return new Vue({
    ...
    async mounted() {
      ...
      setInterval(() => {
        const marker = document.querySelector("#profiler-end")
        if (marker) {
          this.mockresize()
          marker.remove()
        }
    ...
    data: {
      ...
      plugins: {
        ...
        descriptions: {
          ...
          "base.repositories": "Repositories profiler",
    ...
    computed: {
      ...
      //Endpoint to use for computed profiler
      url() {
        //Plugins enabled
        ...
        return [
          ...
          ...(this.scopes.size
            ? [
              ...
              `          #  - read:org      (for organization related profiler)`,
          ...
          `          token: ${this.scopes.size ? `${"$"}{{ secrets.PROFILER_TOKEN }}` : "NOT_NEEDED"}`,
          ``,
          `          # Options`,
    ...
    methods: {
      ...
      mockresize() {
        ...
        if ((svg) && (svg.getAttribute("height") == 99999)) {
          const height = svg.querySelector("#profiler-end")?.getBoundingClientRect()?.y - svg.getBoundingClientRect()?.y
          if (Number.isFinite(height))
            svg.setAttribute("height", height)
        }
      },
      //Generate profiler and flush cache
      async generate() {
