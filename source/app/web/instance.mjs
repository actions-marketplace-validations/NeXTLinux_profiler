...
import profiler from "../profiler/index.mjs"
import presets from "../profiler/presets.mjs"
import setup from "../profiler/setup.mjs"
...
export default async function({sandbox = false} = {}) {
  ...
  //Sandbox mode
  if (sandbox) {
    console.debug("profiler/app > sandbox mode is specified, enabling advanced features")
  ...
  for (const plugin of Object.keys(Plugins).filter(x => !["base", "core"].includes(x))) {
    ...
    if (conf.settings["plugins.default"])
      settings.plugins[plugin].enabled = settings.plugins[plugin].enabled ?? (console.debug(`profiler/app > auto-enabling ${plugin}`), true)
    ...
    if (mock) {
      ...
      for (const token of tokens) {
        if ((!settings.plugins[plugin][token]) || (mock === "force")) {
          console.debug(`profiler/app > using mocked token for ${plugin}.${token}`)
  ...
  if (((mock) && (!conf.settings.token)) || (mock === "force")) {
    console.debug("profiler/app > using mocked token")
  ...
  const uapi = session => {
    ...
    if (authenticated.has(session)) {
      ...
      console.debug(`profiler/app/session/${login} > authenticated with session ${session.substring(0, 6)}, using custom octokit`)
      return {login, graphql: octokit.graphql.defaults({headers: {authorization: `token ${token}`}}), rest: new OctokitRest.Octokit({auth: token})}
    }
    else if (session) {
      console.debug(`profiler/app/session > unknown session ${session.substring(0, 6)}, using default octokit`)
    }
    return null
  }
  ...
  if (!conf.settings.notoken) {
    const refresh = async () => {
      try {
        ...
        console.debug("profiler/app > failed to update remaining requests")
      }
    }
    await refresh()
  ...
  app.get("/.extras", limiter, async (req, res) => {
    if ((authenticated.has(req.headers["x-profiler-session"])) && (conf.settings.extras?.logged)) {
  ...
  app.get("/.requests", limiter, async (req, res) => {
    try {
      const session = req.headers["x-profiler-session"]
      ...
      if (custom) {
        try {
          ...
          if (error.status === 401) {
            console.debug(`profiler/app/oauth > session ${session.substring(0, 6)} is not valid anymore, removing it from cache`)
  ...
  app.get("/.uncache", limiter, (req, res) => {
    ...
    if (token) {
      if (actions.flush.has(token)) {
        console.debug(`profiler/app/${actions.flush.get(token)} > flushed cache`)
  ...
  if (conf.settings.oauth) {
    console.debug("profiler/app/oauth > enabled")
    ...
    app.get("/.oauth/authenticate", (req, res) => {
      ...
      console.debug(`profiler/app/oauth > request ${state}`)
    ...
    app.get("/.oauth/authorize", async (req, res) => {
      ...
      if ((!state) || (!states.has(state))) {
        console.debug("profiler/app/oauth > 400 (invalid state)")
        return res.status(400).send("Bad request: invalid state")
      }
      //OAuth
      try {
        //Authorize user
        console.debug("profiler/app/oauth > authorization")
        ...
        console.debug(`profiler/app/oauth > authorization success for ${login}`)
        const session = crypto.randomBytes(128).toString("hex")
        authenticated.set(session, {login, token})
        console.debug(`profiler/app/oauth > created session ${session.substring(0, 6)}`)
        //Redirect user back
        const {from} = states.get(state)
        return res.redirect(`/.oauth/redirect?${new url.URLSearchParams({to: from, session})}`)
      }
      catch {
        console.debug("profiler/app/oauth > authorization failed")
    ...
    app.get("/.oauth/revoke/:session", limiter, async (req, res) => {
      ...
      if (authenticated.has(session)) {
        ...
        try {
          ...
          authenticated.delete(session)
          console.debug(`profiler/app/oauth > deleted session ${session.substring(0, 6)}`)
  ...
  if (conf.settings.modes.includes("insights")) {
    console.debug("profiler/app > setup insights mode")
    //Legacy routes
    ...
    app.get("/insights/query/:login/:plugin/", async (req, res) => {
      ...
      if (!/^[-\w]+$/i.test(login)) {
        console.debug(`profiler/app/${login}/insights > 400 (invalid username)`)
        return res.status(400).send("Bad request: username seems invalid")
      }
      //Check plugin
      const plugin = req.params.plugin?.replace(/[\n\r]/g, "")
      if (!/^\w+$/i.test(plugin)) {
        console.debug(`profiler/app/${login}/insights > 400 (invalid plugin name)`)
    ...
    app.get("/insights/query/:login/", ...middlewares, async (req, res) => {
      ...
      try {
        ...
        if ((!debug) && (!mock) && (pending.has(`insights.${login}`))) {
          console.debug(`profiler/app/${login}/insights > awaiting pending request`)
        ...
        if ((!debug) && (cached) && (cache.get(`insights.${login}`))) {
          console.debug(`profiler/app/${login}/insights > using cached results`)
          return res.send(cache.get(`insights.${login}`))
        }
        //Compute profiler
        console.debug(`profiler/app/${login}/insights > compute insights`)
        const callbacks = {
          async plugin(login, plugin, success, result) {
            console.debug(`profiler/app/${login}/insights/plugins > ${plugin} > ${success ? "success" : "failure"}`)
            cache.put(`insights.${login}.${plugin}`, result)
          },
        }
        ;(async () => {
          try {
            const json = await profiler.insights({login}, {...api, ...uapi(req.headers["x-profiler-session"]), conf, callbacks}, {Plugins, Templates})
            //Cache
            ...
            console.error(`profiler/app/${login}/insights > error > ${error}`)
          }
        })()
        console.debug(`profiler/app/${login}/insights > accepted request`)
        return res.status(202).json({processing: true, plugins: Object.keys(profiler.insights.plugins)})
      }
      //Internal error
      catch (error) {
        //Not found user
        if ((error instanceof Error) && (/^user not found$/.test(error.message))) {
          console.debug(`profiler/app/${login} > 404 (user/organization not found)`)
        ...
        if ((error instanceof Error) && (/this may be the result of a timeout, or it could be a GitHub bug/i.test(error.errors?.[0]?.message))) {
          console.debug(`profiler/app/${login} > 502 (bad gateway from GitHub)`)
        ...
        //General error
        console.error(error)
        return res.status(500).send("Internal Server Error: failed to process profiler correctly")
      }
      finally {
        solve?.()
  ...
  if (conf.settings.modes.includes("embed")) {
    console.debug("profiler/app > setup embed mode")
    //Static routes
    ...
    app.get("/:login/:repository?", ...middlewares, async (req, res, next) => {
      ...
      if ((login.startsWith(".")) || (login.includes("/")))
        return next()
      if (!/^[-\w]+$/i.test(login)) {
        console.debug(`profiler/app/${login} > 400 (invalid username)`)
        return res.status(400).send("Bad request: username seems invalid")
      }
      //Allowed list check
      if ((restricted.length) && (!restricted.includes(login))) {
        console.debug(`profiler/app/${login} > 403 (not in allowed users)`)
        return res.status(403).send("Forbidden: username not in allowed list")
      }
      //Prevent multiples requests
      if ((!debug) && (!mock) && (pending.has(login))) {
        console.debug(`profiler/app/${login} > awaiting pending request`)
      ...
      if ((!debug) && (cached) && (cache.get(login))) {
        console.debug(`profiler/app/${login} > using cached image`)
      ...
      if ((maxusers) && (cache.size() + 1 > maxusers)) {
        console.debug(`profiler/app/${login} > 503 (maximum users reached)`)
        return res.status(503).send("Service Unavailable: maximum number of users reached, only cached profiler are available")
      }
      //Repository alias
      if (repository) {
        console.debug(`profiler/app/${login} > compute repository profiler`)
      ...
      try {
        //Prepare settings
        const q = req.query
        console.debug(`profiler/app/${login} > ${util.inspect(q, {depth: Infinity, maxStringLength: 256})}`)
        const octokit = {...api, ...uapi(req.headers["x-profiler-session"])}
        let uconf = conf
        if ((octokit.login) && (conf.settings.extras?.logged) && (uconf.settings.extras?.features !== true)) {
          console.debug(`profiler/app/${login} > session is authenticated, adding additional permissions ${conf.settings.extras.logged}`)
        ...
        //Preset
        if ((q["config.presets"]) && ((uconf.settings.extras?.features?.includes("profiler.setup.community.presets")) || (uconf.settings.extras?.features === true) || (uconf.settings.extras?.default))) {
          console.debug(`profiler/app/${login} > presets have been specified, loading them`)
        ...
        const {rendered, mime} = await profiler({login, q}, {
          ...octokit,
          plugins,
          conf: uconf,
      ...
      //Internal error
        ...
        if ((error instanceof Error) && (/^unsupported template$/.test(error.message))) {
          console.debug(`profiler/app/${login} > 400 (bad request)`)
        ...
        if ((error instanceof Error) && (/^not supported for: [\s\S]*$/.test(error.message))) {
          console.debug(`profiler/app/${login} > 406 (Not Acceptable)`)
  ...
  if (conf.settings.control?.token) {
    const middleware = (req, res, next) => {
      console.log(`profiler/app/control > ${req.url.replace(/[\n\r]/g, "")}`)
    ...
    app.post("/.control/stop", limiter, middleware, async (req, res) => {
      console.debug("profiler/app/control > instance will be stopped in a few seconds")
