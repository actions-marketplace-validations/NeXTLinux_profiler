...
export default async function metadata({log = true, diff = false} = {}) {
  ...
  const __descriptor = path.join(__profiler, "action.yml")
  ...
  let Plugins = {}
  logger("profiler/metadata > loading plugins metadata")
  for (const name of await fs.promises.readdir(__plugins)) {
    ...
    switch (name) {
      case "community": {
        ...
        for (const name of await fs.promises.readdir(___plugins)) {
          if (!(await fs.promises.lstat(path.join(___plugins, name))).isDirectory())
            continue
          logger(`profiler/metadata > loading plugin metadata [community/${name}]`)
      ...
      default:
        logger(`profiler/metadata > loading plugin metadata [${name}]`)
  ...
  logger(`profiler/metadata > loaded [${Object.keys(Plugins).join(", ")}]`)
  //Load templates metadata
  let Templates = {}
  logger("profiler/metadata > loading templates metadata")
  for (const name of await fs.promises.readdir(__templates)) {
    if (!(await fs.promises.lstat(path.join(__templates, name))).isDirectory())
      continue
    logger(`profiler/metadata > loading template metadata [${name}]`)
...
metadata.plugin = async function({__plugins, __templates, name, logger}) {
  try {
    ...
    //Inputs parser
    {
      meta.inputs = function({data: {user = null} = {}, q, account}, defaults = {}) {
        //Support check
        if (!account)
          console.debug(`profiler/inputs > account type not set for plugin ${name}!`)
        if (account !== "bypass") {
              ...
              switch (type) {
                ...
                case "array": {
                  try {
                    ...
                    logger(`profiler/inputs > failed to decode uri : ${value}`)
                    value = defaulted
                  }
                  ...
                  logger(`profiler/inputs > used ${used} format to decode ${value}`)
                  return parsed
                }
                //String
                case "string": {
                ...
                case "json": {
                  ...
                  try {
                    ...
                    try {
                      ...
                      logger(`profiler/inputs > failed to parse json : ${value}`)
        ...
        logger(`profiler/inputs > ${name} > ${JSON.stringify(result)}`)
        return result
      }
    ...
    //Extra features parser
    {
      meta.extras = function(input, {extras = {}, error = true} = {}) {
        ...
        try {
          ...
          if (!required)
            return true
          console.debug(`profiler/extras > ${name} > ${key} > require [${required}]`)

          //Legacy handling
          const enabled = Array.isArray(extras) ? extras : (extras?.features ?? extras?.default ?? (typeof extras === "boolean" ? extras : false))
          if (typeof enabled === "boolean") {
            console.debug(`profiler/extras > ${name} > ${key} > extras features is set to ${enabled}`)
            if (!enabled)
              throw new Error()
            return enabled
          }
          if (!Array.isArray(required)) {
            console.debug(`profiler/extras > ${name} > ${key} > extras is not a permission array, skipping`)
            return false
          }

          //Legacy options handling
          if (!Array.isArray(enabled))
            throw new Error(`profiler/extras > ${name} > ${key} > extras.features is not an array`)
          if (extras.css) {
            console.warn(`profiler/extras > ${name} > ${key} > extras.css is deprecated, use extras.features with "profiler.run.puppeteer.user.css" instead`)
            enabled.push("profiler.run.puppeteer.user.css")
          }
          if (extras.js) {
            console.warn(`profiler/extras > ${name} > ${key} > extras.js is deprecated, use extras.features with "profiler.run.puppeteer.user.js" instead`)
            enabled.push("profiler.run.puppeteer.user.js")
          }
          if (extras.presets) {
            console.warn(`profiler/extras > ${name} > ${key} > extras.presets is deprecated, use extras.features with "profiler.setup.community.presets" instead`)
            enabled.push("profiler.setup.community.presets")
          }

          //Check permissions
          const missing = required.filter(permission => !enabled.includes(permission))
          if (missing.length > 0) {
            console.debug(`profiler/extras > ${name} > ${key} > missing permissions [${missing}]`)
            throw new Error()
          }
          return true
        }
          ...
          if (!error) {
            console.debug(`profiler/extras > ${name} > ${key} > skipping (no error mode)`)
            return false
          }
        ...
        for (const key of Object.keys(inputs)) {
          ...
          if (env.ghactions) {
            ...
            try {
              ...
              logger(`profiler/inputs > failed to decode uri for ${key}: ${value}`)
              value = "<default-value>"
            }
          }
          else {
          ...
          if ((key in preset) && (unspecified)) {
            logger(`profiler/inputs > ${key} has been set by preset value`)
            q[key] = preset[key]
          }
          //From defaults
          else if (unspecified) {
            logger(`profiler/inputs > ${key} has been set by default value`)
            q[key] = metadata.inputs[key]?.default
          }
          //From user
          else {
            logger(`profiler/inputs > ${key} has been set by user`)
            q[key] = value
          }
        }
  ...
  <tr>
    ...
    console.debug(`profiler/metadata > failed to load plugin ${name}: ${error}`)
    return null
  }
}
...
metadata.template = async function({__templates, name, plugins}) {
  try {
    ...
    console.debug(`profiler/metadata > failed to load template ${name}: ${error}`)
    return null
  }
}
