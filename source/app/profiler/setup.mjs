...
export default async function({log = true, sandbox = false, community = {}, extras = false} = {}) {
  //Paths
  const __profiler = path.join(path.dirname(url.fileURLToPath(import.meta.url)), "../../..")
  const __statics = path.join(__profiler, "source/app/web/statics")
  const __templates = path.join(__profiler, "source/templates")
  const __plugins = path.join(__profiler, "source/plugins")
  const __package = path.join(__profiler, "package.json")
  const __settings = path.join(__profiler, "settings.json")
  const __modules = path.join(__profiler, "node_modules")

  //Init
  const logger = log ? console.debug : () => null
  logger("profiler/setup > setup")
  const conf = {
    authenticated: null,
    templates: {},
    queries: {},
  ...
  //Load settings
  {
    logger("profiler/setup > load settings.json")
    let fd
    try {
      fd = await fs.promises.open(__settings, "r")
      if (sandbox)
        logger("profiler/setup > load settings.json > skipped because in sandbox is enabled")
      else {
        conf.settings = JSON.parse(`${await fs.promises.readFile(fd)}`)
        logger("profiler/setup > load settings.json > success")
      }
    }
    catch (error) {
      if (error?.code === "ENOENT")
        logger("profiler/setup > load settings.json > (missing)")
      else
        logger(`profiler/setup > load settings.json > (error : ${error})`)
    }
    finally {
      await fd?.close()
    }
  ...
  logger("profiler/setup > load package.json")
  conf.package = JSON.parse(`${await fs.promises.readFile(__package)}`)
  logger("profiler/setup > load package.json > success")

  //Load community templates
  if ((conf.settings.extras?.features?.includes?.("profiler.setup.community.templates")) || (conf.settings.extras?.features === true) || (conf.settings.extras?.default) || (extras) || (sandbox)) {
    if ((typeof conf.settings.community.templates === "string") && (conf.settings.community.templates.length)) {
      logger("profiler/setup > parsing community templates list")
    ...
    if ((Array.isArray(conf.settings.community.templates)) && (conf.settings.community.templates.length)) {
      ...
      logger(`profiler/setup > ${conf.settings.community.templates.length} community templates to install`)
      ...
      for (const template of conf.settings.community.templates) {
        try {
          ...
          logger(`profiler/setup > load community template ${template}`)
          ...
          logger(`profiler/setup > run ${command}`)
          //Clone remote repository
          processes.execSync(command, {stdio: "ignore"})
          //Extract template
          logger(`profiler/setup > extract ${name} from ${repo}@${branch}`)
          ...
          //JavaScript file
          if (trust)
            logger(`profiler/setup > keeping @${name}/template.mjs (unsafe mode is enabled)`)
          else if (fs.existsSync(path.join(__templates, `@${name}`, "template.mjs"))) {
            logger(`profiler/setup > removing @${name}/template.mjs`)
            ...
            if (inherit) {
              logger(`profiler/setup > @${name} extends from ${inherit}`)
              if (fs.existsSync(path.join(__templates, inherit, "template.mjs"))) {
                logger(`profiler/setup > @${name} extended from ${inherit}`)
                await fs.promises.copyFile(path.join(__templates, inherit, "template.mjs"), path.join(__templates, `@${name}`, "template.mjs"))
              }
              else {
                logger(`profiler/setup > @${name} could not extends ${inherit} as it does not exist`)
              }
            }
          }
          else {
            logger(`profiler/setup > @${name}/template.mjs does not exist`)
          }

          //Clean remote repository
          logger(`profiler/setup > clean ${repo}@${branch}`)
          await fs.promises.rm(path.join(__templates, ".community"), {recursive: true, force: true})
          logger(`profiler/setup > loaded community template ${name}`)
        }
        catch (error) {
          logger(`profiler/setup > failed to load community template ${template}`)
          logger(error)
        }
      }
    }
    else {
      logger("profiler/setup > no community templates to install")
    }
  }
  else {
    logger("profiler/setup > community templates are disabled")
  }
  ...
  for (const name of await fs.promises.readdir(__templates)) {
    ...
    if ((!(await fs.promises.lstat(directory)).isDirectory()) || (!fs.existsSync(path.join(directory, "partials/_.json"))))
      continue
    logger(`profiler/setup > load template [${name}]`)
    ...
    logger(`profiler/setup > load template [${name}] > success`)
    //Debug
    if (conf.settings.debug) {
      Object.defineProperty(conf.templates, name, {
        get() {
          logger(`profiler/setup > reload template [${name}]`)
          ...
          logger(`profiler/setup > reload template [${name}] > success`)
  ...
  logger(`profiler/setup > setup > enabled modes ${JSON.stringify(conf.settings.modes)}`)
  ...
  logger(`profiler/setup > setup > allowed outputs ${JSON.stringify(conf.settings.outputs)}`)
  ...
  if (conf.settings.token) {
    try {
      ...
      logger(`profiler/setup > setup > authenticated as ${conf.authenticated}`)
    }
    catch (error) {
      logger(`profiler/setup > setup > could not verify authentication : ${error}`)
    }
  }
  ...
  //Conf
  logger("profiler/setup > setup > success")
  ...
  async plugin(name, {__plugins, Plugins, conf, logger}) {
    ...
    if (fs.existsSync(__queries)) {
      ...
      for (const file of await fs.promises.readdir(__queries)) {
        ...
        logger(`profiler/setup > load query [${name}/${query}]`)
        queries[`_${query}`] = `${await fs.promises.readFile(path.join(__queries, file))}`
        logger(`profiler/setup > load query [${name}/${query}] > success`)
        //Debug
        if (conf.settings.debug) {
          Object.defineProperty(queries, `_${query}`, {
            get() {
              logger(`profiler/setup > reload query [${name}/${query}]`)
              const raw = `${fs.readFileSync(path.join(__queries, file))}`
              logger(`profiler/setup > reload query [${name}/${query}] > success`)
              return raw
            },
          })
        }
