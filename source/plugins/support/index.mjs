...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > support > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > support > started ${await browser.version()}`)
    ...
    //Close browser
    console.debug(`profiler/compute/${login}/plugins > support > closing browser`)
    await browser.close()
