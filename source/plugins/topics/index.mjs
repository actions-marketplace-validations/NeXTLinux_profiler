...
export default async function({login, data, imports, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > topics > searching starred topics`)
    let topics = []
    console.debug(`profiler/compute/${login}/plugins > topics > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > topics > started ${await browser.version()}`)
    const page = await browser.newPage()

    //Iterate through pages
    for (let i = 1; i <= 100; i++) {
      //Load page
      console.debug(`profiler/compute/${login}/plugins > topics > loading page ${i}`)
      ...
      console.debug(`profiler/compute/${login}/plugins > topics > extracted ${starred.length} starred topics`)
      //Check if next page exists
      if (!starred.length) {
        console.debug(`profiler/compute/${login}/plugins > topics > no more page to load`)
        break
      }
      topics.push(...starred)
    }
    ...
    //Close browser
    console.debug(`profiler/compute/${login}/plugins > topics > closing browser`)
    await browser.close()

    //Shuffle topics
    if (shuffle) {
      console.debug(`profiler/compute/${login}/plugins > topics > shuffling topics`)
      topics = imports.shuffle(topics)
    }

    //Limit topics (labels)
    if ((type === "labels") && (limit > 0)) {
      console.debug(`profiler/compute/${login}/plugins > topics > keeping only ${limit} topics`)
    ...
    console.debug(`profiler/compute/${login}/plugins > topics > loading artworks`)
    for (const topic of topics) {
      if (topic.icon) {
        console.debug(`profiler/compute/${login}/plugins > topics > processing ${topic.name}`)
        const {icon} = topic
    ...
    if ((type === "icons") && (limit > 0)) {
      console.debug(`profiler/compute/${login}/plugins > topics > filtering topics with icon`)
