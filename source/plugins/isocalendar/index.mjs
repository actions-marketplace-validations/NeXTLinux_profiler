...
export default async function({login, data, graphql, q, imports, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > isocalendar > computing stats`)
    ...
    //Compute SVG
    console.debug(`profiler/compute/${login}/plugins > isocalendar > computing svg render`)
    const size = 6
    let i = 0, j = 0
    let svg = `
...
async function statistics({login, graphql, queries, start, end, calendar}) {
  ...
  for (let from = new Date(start); from < end;) {
    ...
    //Fetch data from api
    console.debug(`profiler/compute/${login}/plugins > isocalendar > loading calendar from "${from.toISOString()}" to "${dto.toISOString()}"`)
