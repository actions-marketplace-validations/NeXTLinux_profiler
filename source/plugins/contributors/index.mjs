...
export default async function({login, q, imports, data, rest, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > contributors > querying api head and base commits`)
    const ref = {
    ...
    //Get commit activity
    console.debug(`profiler/compute/${login}/plugins > contributors > querying api for commits between [${ref.base?.abbreviatedOid ?? null}] and [${ref.head?.abbreviatedOid ?? null}]`)
    const commits = []
    for (let page = 1;; page++) {
      console.debug(`profiler/compute/${login}/plugins > contributors > loading page ${page}`)
      try {
        const {data: loaded} = await rest.repos.listCommits({...repo, per_page: 100, page})
        if (loaded.map(({sha}) => sha).includes(ref.base?.oid)) {
          console.debug(`profiler/compute/${login}/plugins > contributors > reached ${ref.base?.oid}`)
          commits.push(...loaded.slice(0, loaded.map(({sha}) => sha).indexOf(ref.base.oid)))
          break
        }
        if (!loaded.length) {
          console.debug(`profiler/compute/${login}/plugins > contributors > no more page to load`)
          break
        }
    ...
    console.debug(`profiler/compute/${login}/plugins > contributors > ${commits.length} commits loaded (${start} removed)`)
    ...
    for (const {author: {login, avatar_url: avatar}, commit: {message = "", author: {email = ""} = {}}} of commits) {
      if ((!login) || (!imports.filters.text(login, ignored)) || (!imports.filters.text(email, ignored))) {
        console.debug(`profiler/compute/${login}/plugins > contributors > ignored contributor "${login}"`)
        continue
      }
    ...
    if ((sections.includes("categories")) && (imports.metadata.plugins.contributors.extras("categories", {extras}))) {
      ...
      console.debug(`profiler/compute/${login}/plugins > contributors > cloning ${repository} to temp dir ${path}`)
      ...
      try {
        ...
        console.debug(error)
        console.debug(`profiler/compute/${login}/plugins > contributors > an error occurred while processing ${repository}`)
      }
      finally {
        //Cleaning
        console.debug(`profiler/compute/${login}/plugins > contributors > cleaning temp dir ${path}`)
        await imports.fs.rm(path, {recursive: true, force: true})
      }
    }
