...
export default async function({login, imports, data, q, account}, {enabled = false, token = null, extras = false} = {}) {
  ...
  try {
    ...
    const result = {url: `${protocol}//${host}`, detailed, scores: [], profiler: {}}

    //Load scores from API
    console.debug(`profiler/compute/${login}/plugins > pagespeed > querying api for ${result.url}`)
    ...
    //Perform audit
    console.debug(`profiler/compute/${login}/plugins > pagespeed > performing audit ${categories_required}`)
    ...
    for (const category of categories) {
      ...
      console.debug(`profiler/compute/${login}/plugins > pagespeed > performed audit ${category} (status code ${request.status})`)
    }
    ...
    //Detailed profiler
    if (detailed) {
      console.debug(`profiler/compute/${login}/plugins > pagespeed > performing detailed audit`)
      const request = await imports.axios.get(`https://www.googleapis.com/pagespeedonline/v5/runPagespeed?&url=${url}${token ? `&key=${token}` : ""}`)
      Object.assign(result.profiler, ...request.data.lighthouseResult.audits.profiler.details.items)
      console.debug(`profiler/compute/${login}/plugins > pagespeed > performed detailed audit (status code ${request.status})`)
    }
