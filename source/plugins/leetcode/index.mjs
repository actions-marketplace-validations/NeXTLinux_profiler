...
export default async function({login, q, imports, data, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Languages stats
    {
      console.debug(`profiler/compute/${login}/plugins > leetcode > querying api (languages statistics)`)
    ...
    //Skills stats
    {
      console.debug(`profiler/compute/${login}/plugins > leetcode > querying api (skills statistics)`)
    ...
    //Problems
    {
      console.debug(`profiler/compute/${login}/plugins > leetcode > querying api (problems statistics)`)
    ...
    //Recent submissions
    {
      console.debug(`profiler/compute/${login}/plugins > leetcode > querying api (recent submissions statistics)`)
