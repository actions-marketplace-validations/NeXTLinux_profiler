...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Stackoverflow user profiler
    {
      //Account profiler
      console.debug(`profiler/compute/${login}/plugins > stackoverflow > querying api for user ${user}`)
    ...
    for (const {key, sort} of [{key: "answers-recent", sort: "sort=activity&order=desc"}, {key: "answers-top", sort: "sort=votes&order=desc"}].filter(({key}) => sections.includes(key))) {
      ...
      console.debug(`profiler/compute/${login}/plugins > stackoverflow > querying api for ${key}`)
      ...
      console.debug(`profiler/compute/${login}/plugins > stackoverflow > loaded ${result[key].length} items`)
      //Load related questions
      const ids = result[key].map(({question_id}) => question_id).filter(id => id)
      if (ids) {
        console.debug(`profiler/compute/${login}/plugins > stackoverflow > loading ${ids.length} related items`)
