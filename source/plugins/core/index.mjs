...
export default async function({login, q}, {conf, data, rest, graphql, plugins, queries, account, convert, template, callbacks}, {pending, imports}) {
  ...
  //Base64 images
  if (!_base64) {
    console.debug(`profiler/compute/${login} > base64 for images has been disabled`)
  ...
  console.debug(`profiler/compute/${login} > formatting common profiler`)
  ...
  if (_timezone) {
    ...
    try {
      ...
      console.debug(`profiler/compute/${login} > timezone set to ${timezone.name} (${timezone.offset > 0 ? "+" : ""}${Math.round(timezone.offset / (60 * 60 * 1000))} hours)`)
    }
    catch {
      timezone.error = `Failed to use timezone "${timezone.name}"`
      console.debug(`profiler/compute/${login} > failed to use timezone "${timezone.name}"`)
    }
  }
  ...
  console.debug(`profiler/compute/${login} > animations ${data.animated ? "enabled" : "disabled"}`)

  //Extras features
  const extras = conf.settings?.extras?.features ?? conf.settings?.extras?.default ?? false
  console.debug(`profiler/compute/${login} > extras > ${JSON.stringify(extras)}`)
  ...
  for (const name of Object.keys(imports.plugins)) {
    ...
    pending.push((async () => {
      try {
        console.debug(`profiler/compute/${login}/plugins > ${name} > started`)
        ...
        console.debug(`profiler/compute/${login}/plugins > ${name} > completed`)
      }
      catch (error) {
        console.debug(`profiler/compute/${login}/plugins > ${name} > completed (error)`)
        ...
        await callbacks?.plugin?.(login, name, !data.plugins[name].error, data.plugins[name]).catch(error => console.debug(`profiler/compute/${login}/plugins/callbacks > ${name} > ${error}`))
        return result
      }
    })())
  }
  ...
  if (dflags.includes("--cakeday")) {
    console.debug(`profiler/compute/${login} > applying dflag --cakeday`)
  ...
  if ((dflags.includes("--halloween")) || (dflags.includes("--winter"))) {
    ...
    console.debug(`profiler/compute/${login} > applying dflag --halloween`)
    //Color replacer
  ...
  if (dflags.includes("--error")) {
    console.debug(`profiler/compute/${login} > applying dflag --error`)
