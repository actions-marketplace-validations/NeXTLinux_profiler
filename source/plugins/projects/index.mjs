...
export default async function({login, data, imports, graphql, q, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > projects > querying api`)
    ...
    for (const identifier of repositories) {
      ...
      console.debug(`profiler/compute/${login}/plugins > projects > querying api for ${identifier}`)
      ...
      for (const account of ["user", "organization"]) {
        ...
        try {
          console.debug(`profiler/compute/${login}/plugins > projects > falling back to projects classic for ${identifier}`)
      ...
      console.debug(`profiler/compute/${login}/plugins > projects > registering ${identifier}`)
    ...
    console.debug(`profiler/compute/${login}/plugins > projects > processing ${projects.nodes.length} projects`)
    const list = []
    ...
    //Limit
    console.debug(`profiler/compute/${login}/plugins > projects > keeping only ${limit} projects`)
    list.splice(limit)
