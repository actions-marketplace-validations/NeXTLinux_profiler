...
export default async function({login, imports, data, q, account}, {enabled = false, token = "", extras = false} = {}) {
  ...
  try {
    ...
    //Load user profile
    console.debug(`profiler/compute/${login}/plugins > tweets > loading twitter profile (@${username})`)
    ...
    if (profile?.profile_image_url) {
      console.debug(`profiler/compute/${login}/plugins > tweets > loading profile image`)
      profile.profile_image = await imports.imgb64(profile.profile_image_url)
    }

    //Load tweets
    console.debug(`profiler/compute/${login}/plugins > tweets > querying api`)
    ...
    //Limit tweets
    if (limit > 0) {
      console.debug(`profiler/compute/${login}/plugins > tweets > keeping only ${limit} tweets`)
      tweets.splice(limit)
    }
    ...
    await Promise.all(tweets.map(async tweet => {
      ...
      if (attachments) {
        ...
        if (linked) {
          try {
            ...
            console.debug(`profiler/compute/${login}/plugins > tweets > error while retrieving linked content: ${error.message}`)
          }
        }
      }
      else {
      ...
      //Format text
      console.debug(`profiler/compute/${login}/plugins > tweets > formatting tweet ${tweet.id}`)
