...
export default async function({login, data, queries, imports, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    for (let retried = false; !retried; retried = true) {
      try {
        //Query API
        console.debug(`profiler/compute/${login}/plugins > anilist > querying api (user statistics)`)
    ...
    if ((sections.includes("watching")) || (sections.includes("reading"))) {
      for (const type of medias) {
        for (let retried = false; !retried; retried = true) {
          try {
            //Query API
            console.debug(`profiler/compute/${login}/plugins > anilist > querying api (medias lists - ${type})`)
            ...
            for (const {name, entries} of lists) {
              ...
              //Limit results
              if (limit > 0) {
                console.debug(`profiler/compute/${login}/plugins > anilist > keeping only ${limit} medias`)
    ...
    if (sections.includes("favorites")) {
      for (const type of medias) {
        //Query API
        console.debug(`profiler/compute/${login}/plugins > anilist > querying api (favorites ${type}s)`)
        const list = []
        let page = 1
        let next = false
        do {
          try {
            console.debug(`profiler/compute/${login}/plugins > anilist > querying api (favorites ${type}s - page ${page})`)
    ...
    if (sections.includes("characters")) {
      //Query API
      console.debug(`profiler/compute/${login}/plugins > anilist > querying api (favorites characters)`)
      const characters = []
      let page = 1
      let next = false
      do {
        try {
          console.debug(`profiler/compute/${login}/plugins > anilist > querying api (favorites characters - page ${page})`)
          ...
          for (const {name: {full: name}, image: {medium: artwork}} of nodes) {
            console.debug(`profiler/compute/${login}/plugins > anilist > processing ${name}`)
      ...
      if (limit_characters > 0) {
        console.debug(`profiler/compute/${login}/plugins > anilist > keeping only ${limit_characters} characters`)
...
async function retry({login, imports, error}) {
  if ((error.isAxiosError) && (error.response.status === 429)) {
    ...
    console.debug(`profiler/compute/${login}/plugins > anilist > reached requests limit, retrying in ${delay}s`)
