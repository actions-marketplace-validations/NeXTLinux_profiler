...
export default async function({login, data, rest, imports, q, account}, {enabled = false, extras = false, ...defaults} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > habits > querying api`)
    const events = []
    try {
      for (let page = 1; page <= pages; page++) {
        console.debug(`profiler/compute/${login}/plugins > habits > loading page ${page}`)
        events.push(...(await rest.activity.listEventsForAuthenticatedUser({username: login, per_page: 100, page})).data)
      }
    }
    catch {
      console.debug(`profiler/compute/${login}/plugins > habits > no more page to load`)
    }
    console.debug(`profiler/compute/${login}/plugins > habits > ${events.length} events loaded`)
    ...
    console.debug(`profiler/compute/${login}/plugins > habits > filtered out ${commits.length} push events over last ${days} days`)
    habits.commits.fetched = commits.length

    //Retrieve edited files and filter edited lines (those starting with +/-) from patches
    console.debug(`profiler/compute/${login}/plugins > habits > loading patches`)
    const patches = [
    ...
    {
      //Compute commit days
      console.debug(`profiler/compute/${login}/plugins > habits > searching most active day of week`)
    ...
    //Commit hour
    {
      ...
      console.debug(`profiler/compute/${login}/plugins > habits > searching most active time of day`)
    ...
    //Indent style
    {
      ...
      console.debug(`profiler/compute/${login}/plugins > habits > searching indent style`)
      patches
    ...
    //Average characters per line
    {
      ...
      console.debug(`profiler/compute/${login}/plugins > habits > computing average number of characters per line of code`)
    ...
    if ((charts) && (imports.metadata.plugins.habits.extras("charts", {extras, error: false}))) {
      ...
      console.debug(`profiler/compute/${login}/plugins > habits > searching recently used languages using linguist`)
      if (patches.length) {
        ...
        console.debug(`profiler/compute/${login}/plugins > habits > linguist not available`)
      }
    }

    //Generating charts
    if ((["graph", "chartist"].includes(_charts)) && (imports.metadata.plugins.habits.extras("charts.type", {extras}))) {
      console.debug(`profiler/compute/${login}/plugins > habits > generating charts`)
