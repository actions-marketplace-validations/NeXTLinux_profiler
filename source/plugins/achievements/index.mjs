...
async function total({imports}) {
  if (!total.promise) {
    total.promise = new Promise(async (solve, reject) => {
      //Setup browser
      console.debug("profiler/compute/plugins > achievements > filling total from github.com/search")
      const browser = await imports.puppeteer.launch()
      console.debug(`profiler/compute/plugins > achievements > started ${await browser.version()}`)
      ...
      for (let i = 0; (i < 100) && ((!total.users) || (!total.repositories)); i++) {
        ...
        console.debug(`profiler/compute/plugins > achievements > setup found ${result?.type ?? "(?)"}`)
        if ((result?.type) && (!total[result.type])) {
          const {count, type} = result
          total[type] = Number(count) * 10e5
          console.debug(`profiler/compute/plugins > achievements > set total.${type} to ${total[type]}`)
        }
        await page.close()
      ...
      console.debug("profiler/compute/plugins > achievements > total setup complete")
      return solve()
    })
  }
