...
export default async function({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (await imports.which("licensed")) {
      //Setup for licensed
      console.debug(`profiler/compute/${login}/plugins > licenses > searching dependencies licenses using licensed`)
      const path = imports.paths.join(imports.os.tmpdir(), `${repository.databaseId}`)
      //Create temporary directory
      console.debug(`profiler/compute/${login}/plugins > licenses > creating temp dir ${path}`)
      await imports.fs.rm(path, {recursive: true, force: true})
      await imports.fs.mkdir(path, {recursive: true})
      //Clone repository
      console.debug(`profiler/compute/${login}/plugins > licenses > cloning temp git repository ${repository.url} to ${path}`)
      const git = imports.git(path)
      await git.clone(repository.url, path)
      //Run setup
      if (setup) {
        console.debug(`profiler/compute/${login}/plugins > licenses > running setup [${setup}]`)
      ...
      if (!(await imports.fs.stat(imports.paths.join(path, ".licensed.yml")).then(() => 1).catch(() => 0))) {
        console.debug(`profiler/compute/${login}/plugins > licenses > building .licensed.yml configuration file`)
        ...
        console.debug(`profiler/compute/${login}/plugins > licenses > a .licensed.yml configuration file already exists`)
      }

      //Spawn licensed process
      console.debug(`profiler/compute/${login}/plugins > licenses > running licensed`)
      ...
      //Cleaning
      console.debug(`profiler/compute/${login}/plugins > licensed > cleaning temp dir ${path}`)
      await imports.fs.rm(path, {recursive: true, force: true})
    }
    else {
      console.debug(`profiler/compute/${login}/plugins > licenses > licensed not available`)
    }

    //List licenses properties
    console.debug(`profiler/compute/${login}/plugins > licenses > compute licenses properties`)
    ...
    //Count used licenses
    console.debug(`profiler/compute/${login}/plugins > licenses > computing ratio`)
