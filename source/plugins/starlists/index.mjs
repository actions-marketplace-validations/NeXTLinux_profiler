...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > starlists > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > starlists > started ${await browser.version()}`)
    const page = await browser.newPage()

    //Fetch star lists
    console.debug(`profiler/compute/${login}/plugins > starlists > fetching lists`)
    ...
    console.debug(`profiler/compute/${login}/plugins > starlists > found [${lists.map(({name}) => name)}]`)
    lists = lists
      .filter(({name}) => {
      ...
      .slice(0, limit)
    console.debug(`profiler/compute/${login}/plugins > starlists > extracted ${lists.length} lists`)

    //Compute lists content
    const colors = {}
    for (const list of lists) {
      //Fetch star list content
      console.debug(`profiler/compute/${login}/plugins > starlists > fetching ${list.name}`)
      const repositories = []
      for (let i = 1; i <= (languages ? 100 : 1); i++) {
        console.debug(`profiler/compute/${login}/plugins > starlists > fetching page ${i}`)
        ...
        if (!(await page.evaluate(() => document.querySelector(".next_page"))) || await page.evaluate(() => document.querySelector(".next_page.disabled"))) {
          console.debug(`profiler/compute/${login}/plugins > starlists > reached last page`)
          break
        }
      }
    ...
    //Close browser
    console.debug(`profiler/compute/${login}/plugins > starlists > closing browser`)
    await browser.close()
