...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > screenshot > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > screenshot > started ${await browser.version()}`)
    const page = await browser.newPage()
    await page.setViewport(viewport)
    console.debug(`profiler/compute/${login}/plugins > screenshot > loading ${url}`)
    ...
    switch (mode) {
      case "image": {
        ...
        console.debug(`profiler/compute/${login}/plugins > screenshot > coordinates ${JSON.stringify(clip)}`)
