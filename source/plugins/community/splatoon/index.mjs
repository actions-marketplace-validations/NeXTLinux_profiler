...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false, token, "statink.token": _statink_token} = {}) {
  ...
  try {
    ...
    //Save profile
    {
      ...
      console.debug(`profiler/compute/${login}/plugins > splatoon > saving ${profile}`)
      const parsed = JSON.parse(token)
      if (!parsed?.loginState?.sessionToken)
        throw new Error("Configuration is missing sessionToken")
      if (statink) {
        console.debug(`profiler/compute/${login}/plugins > splatoon > stat.ink integration is enabled`)
        if (_statink_token) {
          parsed.statInkApiKey = _statink_token
          console.debug(`profiler/compute/${login}/plugins > splatoon > stat.ink api key set`)
        }
        else {
          data.warnings.push({warning: {message: '"plugin_splatoon_statink" is set without "plugin_splatoon_statink_token"'}})
          console.debug(`profiler/compute/${login}/plugins > splatoon > stat.ink api key missing`)
        }
      }
    ...
    switch (source) {
      case "mocks":
      case "local":
        console.debug(`profiler/compute/${login}/plugins > splatoon > skipping s3si execution`)
        break
      case "splatnet":
    ...
    console.debug(`profiler/compute/${login}/plugins > splatoon > fetched ${fetched.length} matches`)
