...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > 16personalities > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > 16personalities > started ${await browser.version()}`)
    const page = await browser.newPage()
    console.debug(`profiler/compute/${login}/plugins > 16personalities > loading ${url}`)
