...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false, token} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > stock > querying api for company`)
    ...
    console.debug(`profiler/compute/${login}/plugins > stock > querying api for stock`)
    ...
    //Generating chart
    console.debug(`profiler/compute/${login}/plugins > stock > generating chart`)
