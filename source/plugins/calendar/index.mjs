...
export default async function({login, q, data, imports, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > calendar > processing years ${start} to ${end}`)
    const calendar = {years: []}
    for (let year = start; year <= end; year++) {
      console.debug(`profiler/compute/${login}/plugins > calendar > processing year ${year}`)
      const weeks = []
      ...
      for (let from = new Date(newyear); from < endyear;) {
        ...
        //Fetch data from api
        console.debug(`profiler/compute/${login}/plugins > calendar > loading calendar from "${from.toISOString()}" to "${dto.toISOString()}"`)
