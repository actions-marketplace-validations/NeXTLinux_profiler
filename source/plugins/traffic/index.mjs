...
export default async function({login, imports, data, rest, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > traffic > querying api`)
    ...
    if (rejected.length === promised.length) {
      ...
      rejected.map(({reason}) => console.debug(`profiler/compute/${login}/plugins > traffic > warn > ${reason.message}`))
    }

    //Compute views
    console.debug(`profiler/compute/${login}/plugins > traffic > computing stats`)
