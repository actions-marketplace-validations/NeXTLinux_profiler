...
export default async function({login, data, rest, q, account, imports}, {enabled = false, markdown = "inline", extras = false} = {}) {
  ...
  try {
    ...
    if (q.repo) {
      console.debug(`profiler/compute/${login}/plugins > activity > switched to repository mode`)
    ...
    console.debug(`profiler/compute/${login}/plugins > activity > querying api`)
    const events = []
    try {
      for (let page = 1; page <= pages; page++) {
        console.debug(`profiler/compute/${login}/plugins > activity > loading page ${page}/${pages}`)
      ...
      console.debug(`profiler/compute/${login}/plugins > activity > no more page to load`)
    }
    console.debug(`profiler/compute/${login}/plugins > activity > ${events.length} events loaded`)
