...
export default async function({login, data, graphql, rest, q, queries, imports, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (q.repo) {
      console.debug(`profiler/compute/${login}/plugins > people > switched to repository mode`)
    ...
    console.debug(`profiler/compute/${login}/plugins > people > querying api`)
    const result = Object.fromEntries(types.map(type => [type, []]))
    for (const type of types) {
      //Iterate through people
      console.debug(`profiler/compute/${login}/plugins > people > retrieving ${type}`)
      //Rest
      ...
      //GraphQL
        ...
        let pushed = 0
        do {
          console.debug(`profiler/compute/${login}/plugins > people > retrieving ${type} after ${cursor}`)
      ...
      //Shuffle
      if (shuffle) {
        console.debug(`profiler/compute/${login}/plugins > people > shuffling`)
        imports.shuffle(result[type])
      }
      //Filter out identicons if not wanted
      if (_identicons_hide) {
        console.debug(`profiler/compute/${login}/plugins > people > filtering out user without custom avatarUrl`)
        result[type] = result[type].filter(user => user.avatarUrl?.includes("&u="))
      }
      //Limit people
      if (limit > 0) {
        console.debug(`profiler/compute/${login}/plugins > people > keeping only ${limit} ${type}`)
        result[type].splice(limit)
      }
      //Hide real avatar with identicons if enabled
      if (identicons) {
        console.debug(`profiler/compute/${login}/plugins > people > using identicons`)
        result[type].map(user => user.avatarUrl = `https://github.com/identicons/${user.login}.png`)
      }
      //Convert avatars to base64
      console.debug(`profiler/compute/${login}/plugins > people > loading avatars`)
