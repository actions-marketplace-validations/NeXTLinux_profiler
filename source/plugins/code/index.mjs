...
export default async function({login, q, imports, data, rest, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (q.repo) {
      console.debug(`profiler/compute/${login}/plugins > code > switched to repository mode`)
    ...
    console.debug(`profiler/compute/${login}/plugins > code > querying api`)
    const events = []
    try {
      for (let page = 1; page <= pages; page++) {
        console.debug(`profiler/compute/${login}/plugins > code > loading page ${page}/${pages}`)
        events.push(
          ...[
            ...await Promise.all([
      ...
      console.debug(`profiler/compute/${login}/plugins > code > no more page to load`)
    }
    console.debug(`profiler/compute/${login}/plugins > code > ${events.length} events loaded`)
