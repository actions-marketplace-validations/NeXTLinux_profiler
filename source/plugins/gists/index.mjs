...
export default async function({login, data, graphql, q, imports, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    const gists = []
    {
      ...
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/plugins > gists > retrieving gists after ${cursor}`)
      ...
      while ((pushed) && (cursor))
      console.debug(`profiler/compute/${login}/plugins > gists > loaded ${gists.length} gists`)
    }

    //Iterate through gists
    console.debug(`profiler/compute/${login}/plugins > gists > processing ${gists.length} gists`)
