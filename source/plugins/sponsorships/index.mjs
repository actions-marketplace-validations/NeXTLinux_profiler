...
export default async function({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (sections.includes("amount")) {
      console.debug(`profiler/compute/${login}/plugins > sponsorships > querying total amount spend`)
    ...
    if (sections.includes("sponsorships")) {
      console.debug(`profiler/compute/${login}/plugins > sponsorships > querying sponsorships`)
      {
        const fetched = []
        let cursor = null
        let pushed = 0
        do {
          console.debug(`profiler/compute/${login}/sponsorships > retrieving sponsorships after ${cursor}`)
          ...
          fetched.push(...nodes)
          pushed = nodes.length
          console.debug(`profiler/compute/${login}/sponsorships > retrieved ${pushed} sponsorships events after ${cursor}`)
        }
        while ((pushed) && (cursor))
