...
export default async function({login, data, imports, rest, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > lines > querying api`)
    const repos = {}, weeks = {}
    ...
    console.debug(`profiler/compute/${login}/plugins > lines > computing total diff`)
    response.map(({handle, stats}) => {
      ...
      for (const contributor of contributors) {
        ...
        console.debug(`profiler/compute/${login}/plugins > lines > ${handle}: @${contributor.author.login} +${added} -${deleted} ~${changed}`)
    ...
    if (sections.includes("history")) {
      ...
      if (weeks.length) {
        ...
        console.debug(`profiler/compute/${login}/plugins > lines > no history data`)
        result.history = null
      }
    }
