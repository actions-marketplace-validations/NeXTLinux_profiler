...
export default async function({login, imports, data, q, account}, {enabled = false, token = "", sandbox = false, extras = false} = {}) {
  ...
  try {
    ...
    if ((sandbox) && (_token)) {
      token = _token
      console.debug(`profiler/compute/${login}/plugins > music > overridden token value through user inputs as sandbox mode is enabled`)
    }
    ...
    //Handle mode
    console.debug(`profiler/compute/${login}/plugins > music > processing mode ${mode} with provider ${provider}`)
    switch (mode) {
      //Playlist mode
      case "playlist": {
        //Start puppeteer and navigate to playlist
        console.debug(`profiler/compute/${login}/plugins > music > starting browser`)
        const browser = await imports.puppeteer.launch()
        console.debug(`profiler/compute/${login}/plugins > music > started ${await browser.version()}`)
        const page = await browser.newPage()
        console.debug(`profiler/compute/${login}/plugins > music > loading page`)
        ...
        //Close browser
        console.debug(`profiler/compute/${login}/plugins > music > closing browser`)
        await browser.close()
        //Format tracks
        if (Array.isArray(tracks)) {
          //Tracks
          console.debug(`profiler/compute/${login}/plugins > music > found ${tracks.length} tracks`)
        ...
        switch (provider) {
          ...
          case "spotify": {
            ...
            try {
              ...
              console.debug(`profiler/compute/${login}/plugins > music > requesting access token with spotify refresh token`)
              ...
              console.debug(`profiler/compute/${login}/plugins > music > got access token`)
              //Retrieve tracks
              console.debug(`profiler/compute/${login}/plugins > music > querying spotify api`)
              tracks = []
          ...
          case "lastfm": {
            //API call and parse tracklist
            try {
              console.debug(`profiler/compute/${login}/plugins > music > querying lastfm api`)
              tracks = (await imports.axios.get(`https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${user}&api_key=${token}&limit=${limit}&format=json`, {
                headers: {
                  "User-Agent": "lowlighter/profiler",
                  Accept: "application/json",
                },
          ...
          case "youtube": {
            ...
            try {
              //Request access token
              console.debug(`profiler/compute/${login}/plugins > music > requesting access token with youtube refresh token`)
              ...
              //Retrieve tracks
              console.debug(`profiler/compute/${login}/plugins > music > querying youtube api`)
              tracks = []
    ...
    if (Array.isArray(tracks)) {
      //Limit tracklist
      if (limit > 0) {
        console.debug(`profiler/compute/${login}/plugins > music > keeping only ${limit} tracks`)
        tracks.splice(limit)
      }
      //Convert artworks to base64
      console.debug(`profiler/compute/${login}/plugins > music > loading artworks`)
      for (const track of tracks) {
        console.debug(`profiler/compute/${login}/plugins > music > processing ${track.name}`)
