...
export default async function({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (Number.isNaN(year)) {
      ...
      console.debug(`profiler/compute/${login}/plugins > skyline > year set to ${year}`)
    }
    ...
    console.debug(`profiler/compute/${login}/plugins > skyline > starting browser`)
    const browser = await imports.puppeteer.launch()
    console.debug(`profiler/compute/${login}/plugins > skyline > started ${await browser.version()}`)
    ...
    console.debug(`profiler/compute/${login}/plugins > skyline > loading ${url.replaceAll("${login}", login).replaceAll("${year}", year)}`)
    await page.goto(url.replaceAll("${login}", login).replaceAll("${year}", year), {timeout: 90 * 1000})
    console.debug(`profiler/compute/${login}/plugins > skyline > waiting for initial render`)
    ...
    //Generate gif
    console.debug(`profiler/compute/${login}/plugins > skyline > generating frames`)
