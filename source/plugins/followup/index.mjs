...
export default async function({login, data, computed, imports, q, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if ((indepth) && (imports.metadata.plugins.followup.extras("indepth", {extras}))) {
      console.debug(`profiler/compute/${login}/plugins > followup > indepth`)
      followup.indepth = {repositories: {}}

      //Process repositories
      for (const {name: repo, owner: {login: owner}} of data.user.repositories.nodes) {
        try {
          console.debug(`profiler/compute/${login}/plugins > followup > processing ${owner}/${repo}`)
          ...
          console.debug(`profiler/compute/${login}/plugins > followup > found ${collaborators.length} collaborators`)
          ...
          console.debug(error)
          console.debug(`profiler/compute/${login}/plugins > followup > an error occurred while processing ${owner}/${repo}, skipping...`)
        }
      }
    }
