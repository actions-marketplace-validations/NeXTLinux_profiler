...
export default async function({login, q, imports, graphql, queries, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Load started discussions
    {
      ...
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/discussions > retrieving discussions after ${cursor}`)
        ...
        fetched.push(...nodes)
        pushed = nodes.length
        console.debug(`profiler/compute/${login}/discussions > retrieved ${pushed} discussions after ${cursor}`)
      }
      while ((pushed) && (cursor))
    ...
    //Load comments
    {
      ...
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/discussions > retrieving comments after ${cursor}`)
        ...
        fetched.push(...nodes)
        pushed = nodes.length
        console.debug(`profiler/compute/${login}/discussions > retrieved ${pushed} comments after ${cursor}`)
      }
      while ((pushed) && (cursor))
