...
export default async function({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > sponsors > querying sponsors and goal`)
    ...
    let list = []
    {
      ...
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/sponsors > retrieving sponsors after ${cursor}`)
        ...
        fetched.push(...nodes)
        pushed = nodes.length
        console.debug(`profiler/compute/${login}/sponsors > retrieved ${pushed} sponsors after ${cursor}`)
      }
      while ((pushed) && (cursor))
    ...
    //Query past sponsors
    if (past) {
      console.debug(`profiler/compute/${login}/plugins > sponsors > querying past sponsors`)
      ...
      const users = []
      {
        ...
        let pushed = 0
        do {
          console.debug(`profiler/compute/${login}/sponsors > retrieving sponsors events after ${cursor}`)
          ...
          fetched.push(...nodes)
          pushed = nodes.length
          console.debug(`profiler/compute/${login}/sponsors > retrieved ${pushed} sponsors events after ${cursor}`)
        }
        while ((pushed) && (cursor))
