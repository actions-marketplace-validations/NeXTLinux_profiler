...
export default async function({login, data, imports, q, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Retrieve posts
    console.debug(`profiler/compute/${login}/plugins > posts > processing with source ${source}`)
    let posts = null
    let link = null
    switch (source) {
      //Dev.to
      case "dev.to": {
        ...
        console.debug(`profiler/compute/${login}/plugins > posts > querying api`)
    ...
    if (Array.isArray(posts)) {
      //Limit posts
      if (limit > 0) {
        console.debug(`profiler/compute/${login}/plugins > posts > keeping only ${limit} posts`)
        posts.splice(limit)
      }
      //Cover images
      if (covers) {
        console.debug(`profiler/compute/${login}/plugins > posts > formatting cover images`)
