...
import { D3node } from "../../../app/profiler/utils.mjs"
...
export default async function(login, {locations, sample, imports, token}) {
  ...
  if (token) {
    ...
    for (const location of locations) {
      console.debug(`profiler/compute/${login}/plugins > stargazers > worldmap > looking for ${location}`)
      if (!cache.has(location)) {
        try {
          ...
          console.debug(`profiler/compute/${login}/plugins > stargazers > worldmap > ${location} resolved to ${cache.get(location)}`)
        }
        catch (error) {
          console.debug(`profiler/compute/${login}/plugins > stargazers > worldmap > failed to resolve ${location}: ${error}`)
        }
      }
