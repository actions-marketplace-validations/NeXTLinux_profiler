...
export default async function({login, graphql, data, imports, q, queries, account}, {enabled = false, extras = false, "worldmap.token": _worldmap_token} = {}) {
  ...
  try {
    ...
    if (!days) {
      ...
      console.debug(`profiler/compute/${login}/plugins > stargazers > set days to ${days}`)
    }

    //Retrieve stargazers from graphql api
    console.debug(`profiler/compute/${login}/plugins > stargazers > querying api`)
    ...
    for (const {repository, owner} of repositories) {
      ...
      console.debug(`profiler/compute/${login}/plugins > stargazers > retrieving stargazers of ${repository}`)
      let cursor = null
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/plugins > stargazers > retrieving stargazers of ${repository} after ${cursor}`)
      ...
      while ((pushed) && (cursor))
      console.debug(`profiler/compute/${login}/plugins > stargazers > loaded ${dates.length} stargazers for ${repository}`)
    }
    console.debug(`profiler/compute/${login}/plugins > stargazers > loaded ${dates.length} stargazers in total`)
    ...
    if ((["graph", "chartist"].includes(_charts_type)) && (imports.metadata.plugins.stargazers.extras("charts.type", {extras}))) {
      console.debug(`profiler/compute/${login}/plugins > stargazers > generating charts`)
