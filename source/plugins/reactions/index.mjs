  ...
  try {
    ...
    for (const {type, limit} of [{type: "issueComments", limit: _limit1}, {type: "issues", limit: _limit2}, {type: "repositoryDiscussionComments", limit: _limit3}, {type: "repositoryDiscussions", limit: _limit4}].filter(({limit}) => limit)) {
      ...
      try {
        do {
          //Load issue comments
          console.debug(`profiler/compute/${login}/plugins > reactions > retrieving ${type} before ${cursor}`)
          ...
          console.debug(`profiler/compute/${login}/plugins > reactions > currently at ${fetched.length} ${type} comments`)
          //Applying limit
          if ((fetched.length >= limit) || (filtered.length < edges.length)) {
            fetched.splice(limit)
            console.debug(`profiler/compute/${login}/plugins > reactions > keeping only ${fetched.length} ${type} comments`)
          }
        }
        ...
        console.debug(error)
        console.debug(`profiler/compute/${login}/plugins > reactions > an error occurred while retrieving ${type}`)
      }
    }
    console.debug(`profiler/compute/${login}/plugins > reactions > fetched ${comments.length} comments`)
