...
export default async function({login, q, imports, data, account}, {enabled = false, token, extras = false} = {}) {
  ...
  try {
    ...
    console.debug(`profiler/compute/${login}/plugins > wakatime > range: ${range}`)

    //Querying api and format result (https://wakatime.com/developers#stats)
    console.debug(`profiler/compute/${login}/plugins > wakatime > querying api`)
...
async function pickOnlyGitHubPublicRepos({projects, axios, login, limit}) {
  ...
  for await (const project of projects) {
    if (result.length >= limit)
      break
    try {
      console.debug(`profiler/compute/${login}/plugins > wakatime > checking 'https://github.com/${login}/${project.name}'`)
