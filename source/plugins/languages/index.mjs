...
export default async function({login, data, imports, q, rest, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    if (q.repo) {
      console.debug(`profiler/compute/${login}/plugins > languages > switched to repository mode`)
    ...
    console.debug(`profiler/compute/${login}/plugins > languages > custom colors ${JSON.stringify(colors)}`)
    ...
    console.debug(`profiler/compute/${login}/plugins > languages > processing ${data.user.repositories.nodes.length} repositories`)
    ...
    if ((sections.includes("recently-used")) && (imports.metadata.plugins.languages.extras("indepth", {extras}))) {
      try {
        console.debug(`profiler/compute/${login}/plugins > languages > using recent analyzer`)
        ...
        console.debug(`profiler/compute/${login}/plugins > languages > recent analyzer > ${error}`)
      }
    }
    ...
    if ((indepth) && (imports.metadata.plugins.languages.extras("indepth", {extras}))) {
      try {
        console.debug(`profiler/compute/${login}/plugins > languages > switching to indepth mode (this may take some time)`)
        ...
        console.debug(`profiler/compute/${login}/plugins > languages > indepth analysis processed successfully ${languages.commits} and missed ${languages.missed.commits} commits in ${languages.elapsed.toFixed(2)}m`)
      }
      catch (error) {
        console.debug(`profiler/compute/${login}/plugins > languages > indepth analyzer > ${error}`)
      }
    }
    ...
    for (const stats of [languages.stats, languages.lines, languages["stats.recent"].stats, languages["stats.recent"].lines]) {
      ...
      for (const [language, value] of Object.entries(stats)) {
        if (language.toLocaleLowerCase() in aliases) {
          ...
          console.debug(`profiler/compute/${login}/plugins > languages > ${language} -> ${alias}: ${stats[alias]} (+${value})`)
        }
      }
    }
    ...
    for (const {section, stats = {}, lines = {}, missed = {bytes: 0}, total = 0} of [{section: "favorites", stats: languages.stats, lines: languages.lines, total: languages.total, missed: languages.missed}, {section: "recent", ...languages["stats.recent"]}]) {
      console.debug(`profiler/compute/${login}/plugins > languages > formatting stats ${section}`)
