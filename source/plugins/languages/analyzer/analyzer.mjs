...
import { filters } from "../../../app/profiler/utils.mjs"
...
export class Analyzer {
  ...
  /**Debug log */
  debug(message) {
    return console.debug(`profiler/compute/${this.login}/plugins > languages > ${this.constructor.name.replace(/([a-z])([A-Z])/, (_, a, b) => `${a} ${b.toLocaleLowerCase()}`).toLocaleLowerCase()} > ${message}`)
  }
}
