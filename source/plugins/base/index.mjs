...
export default async function({login, graphql, rest, data, q, queries, imports, callbacks}, conf) {
  //Load inputs
  console.debug(`profiler/compute/${login}/base > started`)
  ...
  console.debug(`profiler/compute/${login}/base > affiliations constraints ${affiliations}`)

  //Skip initial data gathering if not needed
  if ((conf.settings.notoken) || (skip)) {
    await callbacks?.plugin?.(login, "base", true, data).catch(error => console.debug(`profiler/compute/${login}/plugins/callbacks > base > ${error}`))
  ...
  for (const account of ["user", "organization"]) {
    try {
      ...
      console.debug(`profiler/compute/${login}/base > account ${account}`)
      ...
      try {
        ...
        console.debug(`profiler/compute/${login}/base > successfully loaded bulk query`)
      }
      catch {
        console.debug(`profiler/compute/${login}/base > failed to load bulk query, falling back to unit queries`)
        //Query basic fields
        const fields = {
        ...
        for (const field of fields) {
          try {
            ...
            console.debug(`profiler/compute/${login}/base > failed to retrieve ${field}`)
        ...
        for (const field of ["totalCount", "totalDiskUsage"]) {
          try {
            ...
            console.debug(`profiler/compute/${login}/base > failed to retrieve repositories.${field}`)
        ...
        if (account === "user") {
          //Query contributions collection
          {
            ...
            for (const field of fields) {
              try {
                ...
                console.debug(`profiler/compute/${login}/base > failed to retrieve contributionsCollection.${field}`)
          ...
          try {
            ...
            console.debug(`profiler/compute/${login}/base > failed to retrieve contributions calendar`)
      ...
      if (account === "user") {
        if ((indepth) && (imports.metadata.plugins.base.extras("indepth", {...conf.settings, error: false}))) {
          ...
          for (const field of fields) {
            ...
            for (let from = new Date(start); from < end;) {
              ...
              //Fetch data from api
              try {
                console.debug(`profiler/compute/${login}/plugins > base > loading contributions collections for ${field} from "${from.toISOString()}" to "${dto.toISOString()}"`)
                ...
                console.debug(`profiler/compute/${login}/plugins > base > failed to load contributions collections for ${field} from "${from.toISOString()}" to "${dto.toISOString()}"`)
              }
        ...
        //Fallback to load whole commit history rather than last year
          ...
          try {
            console.debug(`profiler/compute/${login}/base > loading user commits history`)
            ...
            console.debug(`profiler/compute/${login}/base > falling back to last year commits history`)
          }
        }
        //Hireable status
        if (hireable) {
          console.debug(`profiler/compute/${login}/base > is hireable`)
      ...
      for (const type of ({user: ["repositories", "repositoriesContributedTo"], organization: ["repositories"]}[account] ?? [])) {
        ...
        do {
          console.debug(`profiler/compute/${login}/base > retrieving ${type} after ${cursor}`)
          const request = {}
          try {
            ...
            console.debug(`profiler/compute/${login}/base > failed to retrieve ${_batch} repositories after ${cursor}, this is probably due to an API timeout, halving batch`)
            _batch = Math.floor(_batch / 2)
            if (_batch < 1) {
              console.debug(`profiler/compute/${login}/base > failed to retrieve repositories, cannot halve batch anymore`)
              throw error
            }
            continue
          }
          ...
          pushed = nodes.length
          console.debug(`profiler/compute/${login}/base > retrieved ${pushed} ${type} after ${cursor}`)
          if (pushed < repositories) {
            console.debug(`profiler/compute/${login}/base > retrieved less repositories than expected, probably no more to fetch`)
            break
          }
        }
        ...
        //Limit repositories
        console.debug(`profiler/compute/${login}/base > keeping only ${repositories} ${type}`)
        data.user[type].nodes.splice(repositories)
        console.debug(`profiler/compute/${login}/base > loaded ${data.user[type].nodes.length} ${type}`)
      }
      //Fetch missing packages count from ghcr.io using REST API (as GraphQL API does not support it yet)
      try {
        console.debug(`profiler/compute/${login}/base > patching packages count if possible`)
        ...
        console.debug(`profiler/compute/${login}/base > patched packages count (added ${packages.length} from ghcr.io)`)
      }
      catch {
        console.debug(`profiler/compute/${login}/base > failed to patch packages count, maybe read:packages scope was not provided`)
      }
      //Shared options
      ...
      console.debug(`profiler/compute/${login}/base > shared options > ${JSON.stringify(data.shared)}`)
      //Success
      console.debug(`profiler/compute/${login}/base > graphql query > account ${account} > success`)
      await callbacks?.plugin?.(login, "base", true, data).catch(error => console.debug(`profiler/compute/${login}/plugins/callbacks > base > ${error}`))
      return {}
    }
    catch (error) {
      console.debug(`profiler/compute/${login}/base > account ${account} > failed : ${error}`)
      if (/Could not resolve to a User with the login of/.test(error.message)) {
        console.debug(`profiler/compute/${login}/base > got a "user not found" error for account type "${account}" and user "${login}"`)
        console.debug(`profiler/compute/${login}/base > checking next account type`)
        continue
      }
      throw error
    }
  ...
  //Not found
  console.debug(`profiler/compute/${login}/base > no more account type`)
  await callbacks?.plugin?.(login, "base", false, data).catch(error => console.debug(`profiler/compute/${login}/plugins/callbacks > base > ${error}`))
...
const postprocess = {
  //User
  user({login, data}) {
    console.debug(`profiler/compute/${login}/base > applying postprocessing`)
    data.account = "user"
