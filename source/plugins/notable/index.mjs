...
export default async function({login, q, imports, rest, graphql, data, account, queries}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    let contributions = []
    {
      ...
      let pushed = 0
      do {
        console.debug(`profiler/compute/${login}/plugins > notable > retrieving contributed repositories after ${cursor}`)
    ...
    console.debug(`profiler/compute/${login}/plugins > notable > found ${contributions.length} notable contributions`)

    //Indepth
    if ((indepth) && (imports.metadata.plugins.notable.extras("indepth", {extras}))) {
      console.debug(`profiler/compute/${login}/plugins > notable > indepth`)
      ...
      if (types.includes("issue")) {
        ...
        let pushed = 0
        do {
          console.debug(`profiler/compute/${login}/plugins > notable > retrieving user issues after ${cursor}`)
      ...
      if (types.includes("pull_request")) {
        ...
        let pushed = 0
        do {
          console.debug(`profiler/compute/${login}/plugins > notable > retrieving user pull requests after ${cursor}`)
      ...
      for (const contribution of contributions) {
        ...
        try {
          ...
          console.debug(`profiler/compute/${login}/plugins > notable > indepth > successfully processed ${owner}/${repo}`)
        }
        catch (error) {
          console.debug(error)
          console.debug(`profiler/compute/${login}/plugins > notable > indepth > failed to compute for ${owner}/${repo}`)
        }
      }
    }

    //Aggregate contributions
    console.debug(`profiler/compute/${login}/plugins > notable > aggregating results`)
