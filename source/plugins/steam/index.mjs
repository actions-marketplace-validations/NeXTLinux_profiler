...
export default async function({login, q, imports, data, account}, {token, enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Fetch owned games
    console.debug(`profiler/compute/${login}/plugins > steam > fetching owned games`)
      ...
      result.games[section] = await Promise.all(
        games
          ...
          .map(async game => {
            const schema = {}
            try {
              console.debug(`profiler/compute/${login}/plugins > steam > fetching schema for "${game.name}" (${game.id})`)
              ...
              console.debug(`profiler/compute/${login}/plugins > steam > failed to get schema for "${game.name}" (${game.id}) > ${error}`)
            }
            const about = {}
            try {
              console.debug(`profiler/compute/${login}/plugins > steam > fetching details for "${game.name}" (${game.id})`)
              ...
              console.debug(`profiler/compute/${login}/plugins > steam > failed to get details for "${game.name}" (${game.id}) > ${error}`)
            }

            let achievements = []
            const rate = {total: Object.keys(schema).length, achieved: 0}
            try {
              console.debug(`profiler/compute/${login}/plugins > steam > fetching player achievements "${game.name}" (${game.id})`)
              ...
              console.debug(`profiler/compute/${login}/plugins > steam > failed to get player achievements for "${game.name}" (${game.id}) > ${error}`)
            }
    ...
    if (sections.includes("player")) {
      console.debug(`profiler/compute/${login}/plugins > steam > fetching profile info`)
      let {data: {response: {players: [info]}}} = await imports.axios.get(urls.player.summary)
      console.debug(`profiler/compute/${login}/plugins > steam > fetching profile level`)
