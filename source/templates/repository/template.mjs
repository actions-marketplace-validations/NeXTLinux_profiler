...
export default async function({login, q}, {data, rest, graphql, queries, account}, {pending, imports}) {
  ...
  const {repo} = q
  if (!repo) {
    console.debug(`profiler/compute/${login}/${repo} > error, repo was undefined`)
    data.errors.push({error: {message: 'You must pass a "repo" argument to use this template'}})
    return imports.plugins.core(...arguments)
  }
  console.debug(`profiler/compute/${login}/${repo} > switching to mode ${account}`)

  //Retrieving single repository
  console.debug(`profiler/compute/${login}/${repo} > retrieving single repository ${repo}`)
  ...
  //Get commit activity
  console.debug(`profiler/compute/${login}/${repo} > querying api for commits`)
  const commits = []
  for (let page = 1; page < 100; page++) {
    console.debug(`profiler/compute/${login}/${repo} > loading page ${page}`)
    try {
      const {data} = await rest.repos.listCommits({owner: login, repo, per_page: 100, page})
      if (!data.length) {
        console.debug(`profiler/compute/${login}/${repo} > no more page to load`)
        break
      }
      commits.push(...data)
    }
  ...
  console.debug(`profiler/compute/${login}/${repo} > ${commits.length} commits loaded`)
  ...
  try {
    if (await rest.repos.getContent({owner: login, repo, path: "action.yml"})) {
      console.debug(`profiler/compute/${login}/${repo} > this repository seems to be a GitHub action, fetching users using code search`)
